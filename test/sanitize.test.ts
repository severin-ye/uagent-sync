...
describe("config sanitization for export", () => {
  ...
  it("should strip environment variables from MCP config", () => {
    ...
    try {
      ...
      assert.ok(Array.isArray(mcp.testMcp.command));
    } finally {
      if (hadReal) fs.renameSync(realPath + ".santest", realPath);
    }
  });
