...
export function prepareCrystallize(input: {
  ...
  const logText = fs.existsSync(logFile) ? fs.readFileSync(logFile, "utf8") : undefined;
  const log = logText === undefined ? { entries: [] } : JSON.parse(logText) as InstallLog;
  ...
  // Legacy artifacts may have passed an older scanner. Revalidate before any
  // write, including new events that would append to the existing history.
  if (logText !== undefined) assertNoSecrets(logText, logFile);
  assertNoSecrets(JSON.stringify(log), logFile);
  ...
  if (fs.existsSync(journalFile)) {
    ...
    const saved = JSON.parse(journalText) as Journal;
    if (saved.version !== 1 || typeof saved.eventKey !== "string" || !saved.snapshot) throw new Error("Invalid crystallize operation journal");
    // An absent manifest marks interrupted preparation. A present manifest must
    // prove all mandatory outputs were prepared before it can authorize reuse.
    if (Object.hasOwn(saved, "artifacts")) {
      const artifacts = saved.artifacts;
      if (!artifacts || typeof artifacts !== "object" || Array.isArray(artifacts) || Object.getPrototypeOf(artifacts) !== Object.prototype) {
        throw new Error("Invalid crystallize artifact manifest");
      }
      const entries = Object.entries(artifacts);
      const paths = new Set(entries.map(([relative]) => relative.replaceAll("\\", "/")));
      if (entries.length === 0 || ["state/install-log.json", "state/workspace-state.json", "guide/SYNC-GUIDE.md"].some(relative => !paths.has(relative)) ||
          entries.some(([, hash]) => typeof hash !== "string" || !/^[a-f0-9]{64}$/i.test(hash))) {
        throw new Error("Invalid crystallize artifact manifest");
      }
    }
    assertNoSecrets(journalText, journalFile);
    assertNoSecrets(JSON.stringify(saved), journalFile);
  ...
  if (journal?.artifacts && existing && journal.entryId === existing.id) {
    for (const [relative, hash] of Object.entries(journal.artifacts)) {
      ...
      if (digest(content) !== hash) throw new Error(`Crystallize retry refused: prepared artifact changed: ${relative}. Preserve/reconcile it before retrying, or use a new --event-id.`);
