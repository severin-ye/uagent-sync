...
function trustedExtensionRegistries(files: Record<string, string>): { locations: Record<string, ExtensionSource>; registrations: Record<string, ExtensionSource> } {
  ...
  for (const file of Object.keys(files)) {
    ...
    // A registration name is not proof of provenance. Only a config record whose
    // path exactly matches a controlled cache file may inherit the official source.
    if (plugin) locations[file] = plugin[1];
  }
