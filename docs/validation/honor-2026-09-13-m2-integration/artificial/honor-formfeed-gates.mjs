...
for(const [id,tail,expectBlocked] of [['safe','',false],['space','PASSWORD=("SYNTHETIC_NONEMPTY")\n',true],['formfeed','PASSWORD=\f("SYNTHETIC_NONEMPTY")\n',true]])for(const nl of ['LF','CRLF']){
 ...
 rows.push({id,nl,expectBlocked,create,plan,restore,publish,newExternalCalls:newCalls,created,targetWritten,baselineWritten,sourceUnchanged:hash(fs.readFileSync(source.codexHome+'/skills/a.py'))===hash(text),passed:expectBlocked?create.rejected&&plan.rejected&&restore.rejected&&!created&&!targetWritten&&!baselineWritten&&newCalls===0:!create.rejected&&!plan.rejected&&!restore.rejected&&created&&targetWritten&&newCalls===1});
}
