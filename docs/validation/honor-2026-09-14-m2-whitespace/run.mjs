...
env.PATH=path.dirname(exe)+';'+env.PATH;
fs.writeFileSync(path.join(base,label+'-pre-env.json'),JSON.stringify(Object.fromEntries(Object.keys(env).filter(k=>/^(HOME|USERPROFILE|APPDATA|LOCALAPPDATA|CODEX_HOME|XDG_CONFIG_HOME|TMP|TEMP|npm_config_|UAGENT_|OPENCODE_|NODE_)/i.test(k)).map(k=>[k,env[k]])),null,2));
...
const commands=label==='setup'?[['ci',[npm,'ci','--prefix',src,'--ignore-scripts','--include=optional','--no-audit','--no-fund']],['build',[npm,'run','build','--prefix',src]]]:label==='full'?[['full',[npm,'test','--prefix',src]]]:[['focused',['--import','tsx','--test',...focused]]];
const results=[];
